 ...
 * Add breadcrumb.
 *
 * @action otzi_lite_after_header
 *
 * @since Otzi Lite 0.1
 */
if( !function_exists( 'otzi_lite_add_breadcrumb' ) ) :
	function otzi_lite_add_breadcrumb() {
		$breadcrumb_option		= otzi_lite_get_option( 'breadcrumb_option' );
		$breadcrumb_on_homepage	= otzi_lite_get_option( 'breadcrumb_on_homepage' );
		$breadcrumb_seperator	= otzi_lite_get_option( 'breadcrumb_seperator' );

		if( $breadcrumb_option ){
			$delimiter = '<span class="sep">'. $breadcrumb_seperator .'</span><!-- .sep -->'; // delimiter between crumbs

			echo otzi_lite_custom_breadcrumbs( $breadcrumb_on_homepage, $delimiter );
		}
		else {
			return false;
		}
...
add_action( 'otzi_lite_after_header', 'otzi_lite_add_breadcrumb', 10 );
 ...
 * @since Otzi Lite 0.1
 */
if( !function_exists( 'otzi_lite_custom_breadcrumbs' ) ) :
	function otzi_lite_custom_breadcrumbs( $showOnHome, $delimiter ) {
		/* === OPTIONS === */
		$text['home']     = esc_html__( 'Home', 'otzi-lite' ); // text for the 'Home' link
		$text['category'] = esc_html__( '%1$s Archive for %2$s', 'otzi-lite' ); // text for a category page
		$text['search']   = esc_html__( '%1$sSearch results for: %2$s', 'otzi-lite' ); // text for a search results page
		$text['tag']      = esc_html__( '%1$sPosts tagged %2$s', 'otzi-lite' ); // text for a tag page
		$text['author']   = esc_html__( '%1$sView all posts by %2$s', 'otzi-lite' ); // text for an author page
		$text['404']      = esc_html__( 'Error 404', 'otzi-lite' ); // text for the 404 page
		...
		if( is_front_page() ) {
			...
			if( get_query_var( 'paged' ) ) {
				...
				echo sprintf( esc_html__( 'Page %s', 'otzi-lite' ), max( $paged, $page ) );
				if( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) {
					echo ')';
				}
			}
	...
	} // end otzi_lite_breadcrumb_lists
endif;
